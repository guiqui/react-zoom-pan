...
export class ZoomPan extends React.Component<IZoomPanProps, IZoomPanState> {
    ...
    addRemoveMouseListener = (newState, oldState) => {
        if (newState.dragging && !oldState.dragging) {
            ...
            document.addEventListener("touchmove", this.doMouseMove);
            document.addEventListener("touchend", this.doMouseUp);
            ...
            document.removeEventListener("touchmove", this.doMouseMove);
            document.removeEventListener("touchend", this.doMouseUp);
        }
    };
    ...
    doGlobalMouseDown = (e) => {
        if (e.button === 0 || e.type === 'touchstart') {
           const [clientX,clientY] = this.getClientXY(e)
            this.setDraggingPosition(clientX, clientY);
    ...
    doMouseMove = (e) => {
        console.log(this.state.dragging, 'dragging')
        if (this.state.dragging) {
            console.log('Moving');
            e.stopPropagation();
            const [clientX,clientY] = this.getClientXY(e)
            const coor: ICoor = this.adjustCoor(clientX, clientY);
            let x = coor.x;
            let y = coor.y;
    ...
    setDraggingPosition = (clientX, clientY) => {
        ...
        this.draggingPositionX = clientX - container.offsetLeft;
        this.draggingPositionY = clientY - container.offsetTop;
    };
        ...
        return (
            ...
            >
                <div
                    ...
                    onTouchStart={this.doGlobalMouseDown}
